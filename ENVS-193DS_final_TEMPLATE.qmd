...
- Tree height is a continuous predictor variable that is measured in cm. As tree height increases, the probability of nest occurrence most likely increases because the birds will be able to nest farther away from potential predators on the ground. **Finish this with citations**
- Acacia ant species is a categorical predictor variable with 3 levels: Crematogaster sjostedti (AB), Crematogaster mimosae (RRB), and Crematogaster nigriceps (BBR).

## d. Table of models

| Model number | Tree Height (cm) | Ant Species | Predictor list              |  
|--------------|:----------------:|------------:|:---------------------------:|  
...
# model 0: null model
model0 <- lm (
  case_control ~ 1, # fromula: no predictors
  data = nests_clean # data = clean
)
...
# model 2: Tree height
model2 <- lm (
  # formula: case_control as a function of tree height
...
# model 3: Ant species
model3 <- lm (
  # formula: case_control as a function of ant species
...
The best model as determined by Akaike’s Information Criterion (AIC) is model 1; the full model including "case_control" as a function of all predictors (tree height and ant species). This model explains the most variation in the simplest way.
